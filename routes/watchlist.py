...
def _uid():
    """Get Firebase UID from header, fallback to 'local'."""
...
def api_get():
    return jsonify({"ok": True, "results": wl_get_all(_uid())})


@wl_bp.route("/api/watchlist/status/<int:movie_id>")
def api_status(movie_id):
    item = wl_get_one(movie_id, _uid())
...
def api_add():
    ...
    if genres or language or runtime:
        added = wl_add_full(int(movie_id), title, poster, backdrop,
                            rating, release_date, genres, language, runtime, _uid())
    else:
        added = wl_add(int(movie_id), title, poster, backdrop, rating, release_date, _uid())
    item = wl_get_one(int(movie_id), _uid())
...
def api_favorite():
    ...
    if not movie_id:
        return jsonify({"ok": False, "error": "movie_id required"}), 400
    if not wl_get_one(int(movie_id), _uid()):
        wl_add(int(movie_id), d.get("title", ""), d.get("poster", ""),
               d.get("backdrop", ""), d.get("rating", 0), d.get("release_date", ""), _uid())
    result = wl_toggle_favorite(int(movie_id), _uid())
    item   = wl_get_one(int(movie_id), _uid())
...
def api_watched():
    ...
    if not movie_id:
        return jsonify({"ok": False, "error": "movie_id required"}), 400
    result = wl_toggle_watched(int(movie_id), _uid())
    item   = wl_get_one(int(movie_id), _uid())
...
def api_rate():
    ...
    wl_rate(int(movie_id), float(user_rating), _uid())
    item = wl_get_one(int(movie_id), _uid())
    return jsonify({"ok": True, "item": item})
...
def api_notes():
    ...
    if not movie_id:
        return jsonify({"ok": False, "error": "movie_id required"}), 400
    wl_set_notes(int(movie_id), notes, _uid())
    item = wl_get_one(int(movie_id), _uid())
    return jsonify({"ok": True, "item": item})
...
def api_collection():
    ...
    if not movie_id:
        return jsonify({"ok": False, "error": "movie_id required"}), 400
    wl_set_collection(int(movie_id), collection, _uid())
    item = wl_get_one(int(movie_id), _uid())
    return jsonify({"ok": True, "item": item})
...
def api_recent_get():
    return jsonify({"ok": True, "results": recent_get(20, _uid())})
